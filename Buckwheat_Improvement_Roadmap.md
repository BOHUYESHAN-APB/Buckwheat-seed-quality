...
- **工具链**：
  ...
  - 移动端（Android）相关：`android-app/` 提供 CameraX + ONNX Runtime 的实时检测样例，正在迭代中（见下文）。

## 移动应用进展（2025-10-07）

- **单张拍照检测（Photo Capture）状态**：未完成。
  - 说明：实时（Live）推理模块在 `android-app` 中已可运行，能进行每帧 ONNX 推理并叠加检测框。但“单张拍照->保存原图与带框副本并持久化元数据”的端到端工作流尚未完全稳定交付，故暂时不视为已完成交付项。
  - 阻塞点：
    - `MainActivity` 与 `CaptureRepository` 的整合尚需完善（拍照按钮与持久化调用的边界条件、UI 反馈、按压缩放扩展的导入/兼容性曾引起编译与行为差异）。
    - 导出选项（含/不含标注）和去黑边逻辑仍需实现与验证。当前代码已修复若干编译错误（metadata 可空性），但端到端行为验证尚未完成。
    - 需要更多的设备端手动测试，以覆盖不同分辨率、旋转和相机方向的场景。
  - 已做工作：
    - 引入 `CaptureRepository`，实现了原图、带框图与 JSON 元数据的读写接口。
    - 修复若干 Kotlin 可空性问题并成功生成 Debug APK，便于一线设备测试。
  - 下一步计划（短期）：
    1. 在 `MainActivity` 中完成拍照成功回调到 `CaptureRepository.persistCapture` 的稳定调用路径，并给出明确的 UI 成功/失败提示。
    2. 实现图片导出时去除模型 letterbox 填充（黑边）的逻辑，保证导出为原始内容尺寸。
    3. 增加一组设备端测试用例（不同分辨率、旋转、文件格式），并记录失败样本以定位回归。
    4. 在 `ImageManagerActivity` 中增加“切换原图/带框”和“批量导出（含/不含标注）”选项。
  - 交付标准（验收条件）：
    - 单张拍照能在所有目标设备上生成原图（raw）、带框图（annotated）和元数据（JSON），文件保存位置明确且可被内置浏览器访问。
    - 导出功能可选择是否保留检测框且输出图像不含黑边。
