...
**状态**: 部分实现（需设备端验证与若干功能完善）

- 说明：当前应用已实现实时推理与检测框叠加；拍照按钮可以触发捕获并进行离线推理。但“端到端稳定保存原图、带框图及元数据（JSON）并提供无黑边导出”的工作流尚未在所有场景下验证完成，因此标记为“部分实现”。

- 已知问题/限制：
   - 持久化流程（`MainActivity` ↔ `CaptureRepository`）的边界条件需进一步加固，存在在极少数设备/分辨率下未写入 annotated 或 metadata 的情况。
   - 导出图片目前没有统一去除模型 letterbox（黑边）的步骤，导出结果在不同分辨率下可能包含填充区域。
   - 需补充 UI 反馈（保存成功/失败的明确提示）与按压缩放（press-scale）兼容性检查。

- 已完成/已准备工作：
   - 引入并实现 `CaptureRepository`（读写原图/带框图与 JSON metadata 的辅助逻辑）。
   - 修复若干 Kotlin 可空性编译错误并能生成 Debug APK（便于设备测试）。

- 短期下一步：
   1. 在 `MainActivity` 中稳固拍照回调到 `CaptureRepository.persistCapture` 的路径，并在 UI 上提供清晰的成功/失败提示。
   2. 实现导出时去黑边逻辑（根据 metadata 或原始尺寸裁剪填充区域）。
   3. 增加设备端回归测试集（多分辨率、旋转、摄像头前/后）并记录失败样本以便修复。
